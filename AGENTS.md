...
- Preserve the approved Explore, Map, Nearby, Saved, and More mobile navigation unless a requested UX change explicitly replaces it. Saved Places and My Trips are local-first features stored on the device; do not add accounts or cloud synchronization unless explicitly approved.
